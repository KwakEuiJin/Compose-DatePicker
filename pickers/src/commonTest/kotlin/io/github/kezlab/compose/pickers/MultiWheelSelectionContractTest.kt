...
import io.github.kezlab.compose.pickers.date.DatePickerState
import io.github.kezlab.compose.pickers.date.repairedDateAfter
...
class MultiWheelSelectionContractTest {
    ...
    fun invalidDependentSources_failDuringConfigurationValidation() {
        ...
        assertFailsWith<IllegalArgumentException> {
            io.github.kezlab.compose.pickers.date.validateDatePickerItems(
                state = DatePickerState(initialDate = LocalDate(2025, 2, 1)),
                items = invalidDateItems
            )
        }
        assertFailsWith<IllegalArgumentException> {
            io.github.kezlab.compose.pickers.time.validateTimePickerItems(
