...
- `pr-verification.yml` runs automatically on every PR to `main` and is the required hosted gate: diff hygiene, `:pickers:desktopTest`, `:pickers:testDebugUnitTest`, `:screenshot-tests:validateDebugScreenshotTest`, and `:pickers:checkKotlinAbi`. The full multiplatform/emulator matrix in `integration-build-test.yml` stays `workflow_dispatch`-only because it is too slow for every push; run it when hosted iOS, Wasm, or managed-device evidence is explicitly requested. Merge PRs once `pr-verification.yml` passes, relevant local verification passes, and GitHub reports the PR as `MERGEABLE`.
...
- **`pr-verification.yml`**: Runs on every `pull_request` to `main`. Three jobs: repository hygiene (`git diff --check`), Linux tests (`:pickers:desktopTest`, `:pickers:testDebugUnitTest`, `:screenshot-tests:validateDebugScreenshotTest`, `:sample:compileDebugKotlinAndroid`), and the public API gate (`:pickers:checkKotlinAbi` on macOS, because the klib dump covers the iOS targets).
